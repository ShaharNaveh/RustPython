...
CPYTHON_PATH = (
    pathlib.Path(__file__).parents[4] / "cpython"  # Local filesystem path of cpython
)
    ...
    def rust_code(self) -> str:
        ...
        for inst in self:
            ...
            if oparg is None:
                ...
                enum_variants.append(f"{name}(crate::Arg<{oparg}>) = {opcode}")
...
pub const fn num_{direction}<T: crate::OpArgType>(self, oparg: T) -> u32 {{
    match self {{
        {arms}
    }}
}}
